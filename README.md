...
当前版本为 `1.0.10 (11)`，补充熄屏后的电池优化设置、独立恢复唤醒、调度迟到诊断和通知中的重试计划。改动与验收见 [`docs/FIX_1.0.10.md`](docs/FIX_1.0.10.md)。[本次 CI](https://github.com/zjjxwpstcnsm-gif/agnes-studio/actions/runs/34542975830) 的 53 项测试全部通过，Lint 无错误，交付后台版 APK 的固定签名校验通过。
...
5. 设置页标题下应显示 `Agnes Studio 1.0.10 (11)`。

覆盖安装必须复用旧版签名密钥。本次恢复的源码不包含旧版 `debug.keystore`，且已确认本次 CI 安装包的签名与旧版不同，因此不能覆盖旧 APK。请勿为解决签名冲突而直接卸载旧版，否则会删除其私有区内的对话、队列与素材。

本次另提供可并存安装的「Agnes Studio 后台版」（`com.ppailab.agnesstudio.background`），无需卸载旧应用。新旧数据独立，旧任务和历史仍在旧应用。后台版交付包使用单独保存的固定私有签名；后续交付必须复用该签名，不可直接拿 CI 临时签名包覆盖。
