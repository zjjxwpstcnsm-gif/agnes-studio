...
- 队列支持全部、进行中、失败和已取消筛选
- 已结束的生成记录支持单条删除；队列「清理」支持确认后批量删除，同时清除调用日志、应用内图片/视频结果和下载残留，保留参考素材及其他任务
...
5. 设置页标题下应显示 `Agnes Studio 1.0.12 (13)`。

覆盖安装必须复用旧版签名密钥。交付的 1.0.12 后台版继续复用 1.0.10 / 1.0.11 后台版固定签名，可直接覆盖；CI 临时签名包不能用来覆盖已安装的后台版。早期原包的旧 `debug.keystore` 未找回，不能保证覆盖早期原包。请勿为解决签名冲突而卸载旧版，否则会删除其私有区内的对话、队列与素材。
