...
- 图片和视频使用 SQLite 持久队列 + 直接启动的 dataSync 前台服务；提交后可切换其他 App，上传、创建和轮询继续执行，完成后自动停止通知和唤醒锁
- WorkManager + 可在 idle 期间触发的非精确闹钟负责系统中断后的持久恢复；日常限流等待与 30 秒轮询在前台服务内连续处理
- 设置页提供电池优化放行与通知设置；未放行时队列页显示提醒。系统强行停止或厂商禁止后台运行仍可能延迟任务
...
- 队列支持全部、进行中、失败和已取消筛选
- 图片页、视频页和队列中的每条生成记录都有“复制提示词”和“复制并生成”按钮；后者以原参数、原素材直接新建任务，保留原记录，自动遵守限流并在发送时刷新过期的临时素材链接；详细参数快照与任务 JSON 仍可单独查看
...
当前版本为 `1.0.11 (12)`，新增历史图片/视频请求一键复制并生成，保留 1.0.10 的后台恢复能力。改动与验收见 [`docs/FIX_1.0.11.md`](docs/FIX_1.0.11.md)。交付后台版复用 1.0.10 的固定证书，可覆盖安装并保留历史。[Android CI](https://github.com/zjjxwpstcnsm-gif/agnes-studio/actions/runs/34741047791) 已通过 64 项测试，Lint 无错误；最终 APK 的 v2/v3 签名、包名与版本已校验。
...
4. 在「后台持续生成」中允许后台运行并开启任务通知；荣耀 / 华为设备还需允许应用启动管理中的自启动、关联启动、后台活动。
5. 设置页标题下应显示 `Agnes Studio 1.0.11 (12)`。

覆盖安装必须复用旧版签名密钥。交付的 1.0.11 后台版复用 1.0.10 后台版固定签名，可直接覆盖；CI 临时签名包不能用来覆盖已安装的后台版。早期原包的旧 `debug.keystore` 未找回，不能保证覆盖早期原包。请勿为解决签名冲突而卸载旧版，否则会删除其私有区内的对话、队列与素材。

从 1.0.10 起提供可并存安装的「Agnes Studio 后台版」（`com.ppailab.agnesstudio.background`），无需卸载旧应用。新旧数据独立，旧任务和历史仍在旧应用。后台版交付包使用单独保存的固定私有签名；后续交付必须复用该签名，不可直接拿 CI 临时签名包覆盖。
