...
| `GenerationQueue` | 用户操作启动前台服务、唤醒处理器、调度持久恢复 |
| `GenerationQueueService` | 独立于 Activity 连续处理生成任务、通知与唤醒锁生命周期 |
...
`SENDING` 与 `PROCESSING` 都是可恢复状态。进程中断后，前台服务或恢复 Worker 会优先拾取它们；已有 `video_id` 的视频只恢复轮询，不重复创建。常规轮询固定间隔 30 秒，轮询异常进入重试时也应用 30 秒最小间隔。
...
用户提交或恢复可见界面时直接启动 dataSync 前台服务，在退出 Activity 后继续执行。
服务持有同一个 QueueProcessor 的 drain 锁，处理器在限流和轮询到期前等待可唤醒信号；
新提交、取消会立即唤醒，普通切后台不取消协程。队列清空后服务停止并释放通知及唤醒锁。
WorkManager 是持久恢复入口：前台服务运行时立即退出；否则只处理已到期任务，
再安排一次后继恢复请求，不长期占用 Worker 等待轮询窗口。
平台的强行停止和 dataSync 总时长限制仍然适用。
